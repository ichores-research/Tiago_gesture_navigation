...
class ServiceClient():
    ...
    RESPONSE_TIMEOUT = 30000  # in ms (how long to wait to receive a response)
    ...
    def __init__(self, port=242424, addr="127.0.0.1", protocol="tcp"):
        ...
        self.__addr = str(protocol + "://" + addr + ":" + str(port)) # full address ~ sort of like a service name/identifier
        print("Creating service client on %s" % self.__addr)
        self._connect()
        self.__lock = Lock()
